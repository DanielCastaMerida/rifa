...
const validateNewUser = (req, res, next) => {
  try {
    ...
    if (!validar_password(password)) {
      req.flash('message', 'Contraseña erronea: Minimo 8 caracteres - ' + 'Maximo 15 - ' + 'Al menos una letra mayúscula - ' + 'Al menos una letra minucula - ' + 'Al menos un dígito - ' + 'No espacios en blanco - ' + 'Al menos 1 caracter especial');
      req.flash('username', username);
      req.flash('email', email);
      req.flash('name', name);
      req.flash('lastname', lastname);
  ...
  try {
    ...
    if (order.length === 0) {
      return res.status(401).json({
        message: "Error al encontrar alguna orden en la base de datos"
      });
    }
    next();
  } catch (error) {
...
const validateNewOrden = async (req, res, next) => {
  try {
    ...
    if (phone.length < 7 || phone.length > 10) return res.status(401).json({
      message: "El telefono debe tener minimo 7 y maximo 10 caracteres"
    });
    if (isNaN(phone)) return res.status(401).json({
      message: "Debe ingresar un numero telefonico valido."
    });
    //customerId
    if (!customerId) return res.status(401).json({
      message: "(TEST) para un customerId valido, ingresa un username registrado"
    });
