...
const updateConfigCommentsHome = async (req, res) => {
  ...
  if (config.length === 0) {
    ...
    await Config.findByIdAndUpdate(req.params.id, req.body, {
      new: true
    });
  }
...
const updateConfigPromotion = async (req, res) => {
  ...
  if (config.length === 0) {
    ...
    config.forEach(async element => {
      await Config.findByIdAndUpdate(element._id, req.body, {
        new: true
      });
    });
  }
...
const updateConfigPromotionActive = async (req, res) => {
  ...
  if (config.length === 0) {
    ...
    config.forEach(async element => {
      await Config.findByIdAndUpdate(element._id, {
        promotion: true
      }, {
        new: true
      });
...
module.exports = {
  rendePageConfig,
  updateConfigCommentsHome,
  updateConfigPromotion,
  updateConfigPromotionActive,
  updateConfigPromotionDesactive
};