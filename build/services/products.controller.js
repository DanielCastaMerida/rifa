...
const createProduct = async (req, res) => {
  try {
    ...
    if (!image) {
      newProduct.image = config.ImageUndefined;
    }
    if (!validar_image(image)) {
      newProduct.image = config.ImageUndefined;
    }
...
const getProductById = async (req, res) => {
  try {
    ...
    return res.status(401).json({
      message: "Error al encontrar el producto, ingresa una ID valida"
    });
  }
};
const updateProductById = async (req, res) => {
  try {
    ...
    if (!image) {
      req.body.image = config.ImageUndefined;
    }
    if (!validar_image(image)) {
      req.body.image = config.ImageUndefined;
    }
